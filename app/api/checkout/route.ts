...
export async function POST(request: Request) {
  ...
  for (const item of requested) {
    const id = Number(item.id), quantity = Math.max(1, Math.min(10, Number(item.quantity) || 1));
  ...
  const products = await getSql()`SELECT id, name, price, inventory FROM jd_products WHERE id = ANY(${Array.from(quantities.keys())}) AND active = TRUE`;
  if (products.length !== quantities.size) return Response.json({ error: "One of those pieces is no longer available." }, { status: 400 });
  const unavailable = products.find(product => Number(product.inventory) < (quantities.get(Number(product.id)) || 1));
  if (unavailable) return Response.json({ error: `${unavailable.name} does not have enough inventory.` }, { status: 400 });
  ...
  form.set("mode", "payment"); form.set("success_url", `${origin}/checkout/success?session_id={CHECKOUT_SESSION_ID}`);
  form.set("cancel_url", `${origin}/#pieces`); form.set("customer_creation", "always");
  form.set("billing_address_collection", "required"); form.set("shipping_address_collection[allowed_countries][0]", "US");
  form.set("shipping_address_collection[allowed_countries][1]", "MX"); form.set("allow_promotion_codes", "true");
  form.set("metadata[cart]", JSON.stringify(cartMeta));
  const stripeResponse = await fetch("https://api.stripe.com/v1/checkout/sessions", { method: "POST", headers: { Authorization: `Basic ${Buffer.from(`${secret}:`).toString("base64")}`, "Content-Type": "application/x-www-form-urlencoded" }, body: form });
